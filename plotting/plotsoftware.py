...
class ImageGrid:
    ...
    def draw_fullscreen(self, screen):
        ...
        if self.fullscreen_original:
            ...
            filename = os.path.basename(self.fullscreen_image)
            text = self.font.render(filename, True, (255, 255, 255))
            ...
            if event.type == pygame.QUIT:
                ...
                if event.button == 1 and dragging:
                    if drag_start_pos and (
                        ...
                        mouse_pos = pygame.mouse.get_pos()
                        col = int((mouse_pos[0] + grid.scroll_offset[0]) / grid.cell_size)
                        row = int((mouse_pos[1] + grid.scroll_offset[1] - INFO_HEIGHT) / grid.cell_size)
                        
                        if row in grid.grid and col < len(grid.grid[row]) and row >= 1 and col > 0:
                            path = grid.grid[row][col]
